...
simple.sendMessage(from, sound, MessageType.audio, { mimetype: 'audio/mp4', ptt:true})
break
...
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgif})
   ...
   sendButImage(from, pll, `MORISHIMABOT-1 © 2021`, cewe, [                      
          {
...
//>>>>>>>>>>>>>>>>>>>>[ COMMAND ]<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<//
  ...
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgif})
               list = []
            listmenu = [`olshop`,`sewabot`,`groupmenu`,`toolsmenu`,`downloadmenu`,`islammenu`,`animemenu`,`informationmenu`,`searchmenu`,`primbonmenu`,`ownermenu`,`cegan`,`djmenu`,`makermenu`,]
               listmenuu = [`olshop`,`sewabot`,`groupmenu`,`toolsmenu`,`downloadmenu`,`islammenu`,`animemenu`,`informationmenu`,`searchmenu`,`primbonmenu`,`ownermenu`,`cegan`,`djmenu`,`makermenu`,]
               nombor = 1
               startnum = 0
...
tescok = `❏  _*Runtime : ${runtime(process.uptime())}*_
  ...
  「 BAILEYS SELF-BOT」 `
Sendbutdocument(from, tescok, "© Z-BOT8*", fs.readFileSync('./poto/menu.pdf'), {mimetype:Mimetype.pdf, thumbnail:fs.readFileSync('./poto/image/thumbnaildokumen.jpg'), filename:`Z-BOT8.pdf`, pageCount: 9999999 },[{ buttonId: `#c`, buttonText: { displayText: 'SIP,KAK😁' }, type: 1 }], {quoted:fvid, contextInfo: { mentionedJid: [dtod,otod,stod], forwardingScore: 508, isForwarded: true, externalAdReply:{title:`${tampilUcapan} - ${pushname}\nDownload Script`,mediaType:"2",thumbnail:ofrply,mediaUrl:`https://youtu.be/WP66OsAf1W0`}}})
break
...
kentod = `Klik Next Untuk Ke ${command} Selanjutnya`
sendButVideo(from, kentod, `*©Haikal | ©DhaniGanss Official Api*`, buffer, [                      
                                                                 {
...
fakelink('```⏰ wait some more time. . .```')
buffer = await getBuffer(`https://apidhani.herokuapp.com/api/randomvideo/${command}?apikey=NisaaCantik`)
ngen = `Klik Buttonnya Kak ${pushname} Untuk Story Selanjutnya`
sendButVideo(from, ngen, `*©Haikal | ©DhaniGanss Official Api*`, buffer, [                      
...
case 'asupan':
fakelink('```⏰ wait some more time. . .```')
buffer = await getBuffer(`https://apidhani.herokuapp.com/api/asupan/asupan?apikey=NisaaCantik`)
ngen = `Klik Buttonnya Kak ${pushname} Untuk Story Selanjutnya`
sendButVideo(from, ngen, `*©Haikal | ©DhaniGanss Official Api*`, buffer, [                      
{buttonId: `${prefix+command}`,buttonText: {displayText: 'NEXT🔜',},type: 1,},]);
break
...
case 'jjml':
fakelink('```⏰ wait some more time. . .```')
buffer = await getBuffer(`https://apidhani.herokuapp.com/api/randomvideo/jedagjedugml?apikey=NisaaCantik`)
ngen = `Klik Buttonnya Kak ${pushname} Untuk Story Selanjutnya`
sendButVideo(from, ngen, `*©Haikal | ©DhaniGanss Official Api*`, buffer, [                      
{buttonId: `${prefix+command}`,buttonText: {displayText: 'NEXT🔜',},type: 1,},]);
break
...
case 'doraemon':
fakelink('```⏰ wait some more time. . .```')
buffer = await getBuffer(`https://apidhani.herokuapp.com/api/randomimage/doraemon?apikey=NisaaCantik`)
kals = `Klik Buttonnya Kak ${pushname} Untuk Doraemon Selanjutnya`
sendButImage(from, kals, `*©Haikal | ©DhaniGanss Official Api*`, buffer, [                      
{buttonId: `${prefix+command}`,buttonText: {displayText: 'NEXT🔜',},type: 1,},]);
break
...
case 'kucing':
fakelink('```⏰ wait some more time. . .```')
buffer = await getBuffer(`https://apidhani.herokuapp.com/api/randomimage/kucing?apikey=NisaaCantik`)
kals = `Klik Buttonnya Kak ${pushname} Untuk Doraemon Selanjutnya`
sendButImage(from, kals, `*©Haikal | ©DhaniGanss Official Api*`, buffer, [                      
{buttonId: `${prefix+command}`,buttonText: {displayText: 'NEXT🔜',},type: 1,},]);
break
...
case 'wallnime':
fakelink('```⏰ wait some more time. . .```')
buffer = await getBuffer(`https://api.lolhuman.xyz/api/random/${command}?apikey=${humankey}`)
kals = `Klik Buttonnya Kak @${pushname} Untuk ${command} Selanjutnya`
sendButImage(from, kals, `*©Haikal | ©LoLHuman Api*`, buffer, [                      
{buttonId: `${prefix+command}`,buttonText: {displayText: 'NEXT🔜',},type: 1,},]);
break
...
fakelink('```⏰ wait some more time. . .```')
buffer = await getBuffer(`https://api.lolhuman.xyz/api/random/nsfw/${command}?apikey=${humankey}`)
kals = `Klik Buttonnya Kak @${pushname} Untuk ${command} Selanjutnya`
sendButImage(from, kals, `*©Haikal | ©LoLHuman Api*`, buffer, [                      
{buttonId: `${prefix+command}`,buttonText: {displayText: 'NEXT🔜',},type: 1,},]);
break
